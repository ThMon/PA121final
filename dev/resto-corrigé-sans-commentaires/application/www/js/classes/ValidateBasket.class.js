...
ValidateBasket.prototype.load = function() {
    ...
    var order = JSON.stringify(this.items);
    ...
    for (var i = 0; i < this.items.length; i++) {
    	...
    	var tr = $('<tr>');
    	tr.append('<td><img src="'+this.items[i].pict+'"><em>'+this.items[i].name+'</em></td><td class="number">'+this.items[i].quantity+'</td><td class="number">'+this.items[i].salePrice+'</td><td class="number">'+(parseInt(this.items[i].quantity)*parseFloat(this.items[i].salePrice)).toFixed(2)+'</td>');
    	tbody.append(tr);
    }
